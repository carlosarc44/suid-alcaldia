...
;(function ($, document, window) {
	...
	$.fn.userTimeout = function (opts) {
		...
		var defaults = {
			...
			session: 3000,                   // 10 Minutes in Milliseconds, then notification of logout
			force: 30000,                     // 5 Minutes in Milliseconds, then logout
			ui: 'auto',                        // Model Dialog selector (auto, bootstrap, jqueryui)
			debug: false,					   // Shows alerts
			modalTitle: 'Cierre de sesión',     // Modal Title
			modalBody: 'La sesión está siendo suspendida temporalmente debido a inactividad. Por favor elija si desea seguir conectado o cerrar la sesión. De lo contrario, la sesión se cerrará automáticamente.'  // Modal Body
		};
		...
		var modal = function () {
			...
			if (modalUI === 'bootstrap') {
				...
				if (options.timer === true) {
					footer = $('<div class="modal-footer"><button type="button" class="btn btn-success" data-dismiss="modal">Seguir conectado (<span id="countdowntimer">' + seconds + '</span>)</button></div>');
					countDown(seconds);
				} else {
					footer = $('<div class="modal-footer"><button type="button" class="btn btn-success" data-dismiss="modal">Seguir conectado</button></div>');
				}
				
				logoutBtn = $('<button type="button" class="btn btn-blanco" id="logoff">Cerrar sesión</button>');
		...
		var logout = function () {
			...
			if (options.referer !== false) {
				...
				window.location = getBaseUrl() + options.logouturl;
			}
		};
...
$(document).userTimeout({
  logouturl: 'shutdown',
  session: 180000000,//1800000 30 minutos (30*60*1000)
  force: 30000, 
});